...
- `notification_app_be`: reserved backend folder for repository structure compliance.

## Data Flow

1. The frontend requests `/evaluation-service/notifications`.
2. Vite proxies the request to the provided notification API.
3. The API response is normalized for field names such as `ID`, `Type`, `Message`, and `Timestamp`.
4. The all-notifications page sorts records by newest first.
5. The priority page filters unread records, applies type and limit controls, and shows the top ranked records.
6. Viewed IDs and application logs are persisted locally in the browser.
